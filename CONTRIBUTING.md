...
2. 只修改目标插件、Skill、Workflow 及确有必要的共享 SDK 或工具链。
3. 提升目标扩展的语义版本，填写简洁的中文 `release_notes`，并确认清单里的 `distribution_targets` 与本次要发的落点一致。
...
- 不提交 `.hmpkg`、`.hmskill`、`.hmwf`、`extension-lock.json`、可执行文件、校验和或 Agent 草稿数据。
